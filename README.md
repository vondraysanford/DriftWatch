...
**Live demo:** [driftwatch.vondraysanford.com](https://driftwatch.vondraysanford.com/), the monitoring dashboard, reading the production prediction log. The API behind it scales to zero, so the first numbers take about half a minute to arrive.
...
**Dashboard**, live at [driftwatch.vondraysanford.com](https://driftwatch.vondraysanford.com/) (Cloudflare Pages, so the page loads instantly and only its numbers wait out the 32.7 s cold start of the API, which is the price of $0 idle) and also served by the container itself at [/dashboard](https://ca-driftwatch.nicewater-47c03558.eastus2.azurecontainerapps.io/dashboard/) with `/api/*` behind it. Tiles for the champion, the latest verdict, ROC-AUC on the replayed regime, prediction volume, and the latest deployment; predictions per hour stacked by regime; the failure-probability distribution against the operating threshold; drift share per regime across the detector's runs against the 30% line; the champion's ROC-AUC on labeled traffic with promotions marked; and tables for champion-vs-challenger, deployments, and recent predictions. Every chart has a table twin, tooltips on every mark, one filter row, light and dark modes behind the same toggle and stored choice as the portfolio site, and the two-series palette validated for color-vision-deficiency separation in both modes. It reads the same blob-stored log and monitoring feeds the pipeline writes, cached for a minute. Styled to the portfolio site's design system, with the site's own navigation bar (same links, theme toggle, and reading-progress hairline), and built once for two homes: inside the serving container at `/dashboard`, and on Cloudflare Pages at its own subdomain with `VITE_API_BASE_URL` pointing at the API (the Container App's `CORS_ALLOW_ORIGINS` environment variable, set in `infra/containerapp.bicep`, allows that origin for the read-only routes). Performance on labeled traffic is attributed to the champion's own predictions, because a window spanning a promotion holds two models' output and pooling them once produced a "champion" score that belonged to neither.
