...
*Finishing touches, 2026-09-04:* the dashboard restyled to the vondraysanford.com design system (tokens, fonts, title block, drafting grid; chart colors re-validated in both modes), live on Cloudflare Pages at `https://driftwatch.vondraysanford.com/` like the sibling projects, verified from the public URL with the browser's cross-origin calls succeeding against the deployed policy (`VITE_API_BASE_URL` to the Container App, `VITE_BASE=/`, and `CORS_ALLOW_ORIGINS` as an environment variable on the Container App, set in `infra/containerapp.bicep` so it survives redeploys, the same place the sibling projects keep their allowed origin), and a monitoring bug fixed that the dashboard exposed: the first scheduled verdict after the promotion pooled predictions from v1 and v2 into one "champion" ROC-AUC of 0.62. Performance is now attributed to the champion's own predictions (`--champion-version`, read from the registry in `drift.yml`); the corrected run scores v2 at 0.9934 with 1,702 v1 predictions excluded. The three scheduled verdicts produced by the pooled computation before the fix reached `main` were removed from the feed and replaced with corrected runs carrying a note; every scheduled run after the fix stands as published.
