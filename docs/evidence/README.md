...
| `phase-5/deploy-run-9-green-model-version-2.png` | Run #9 finished green in 6m 51s with "Model version: 2, Promotion: 2". Also the evidence of a bug this run exposed: the image tag is the commit SHA, identical to the previous push, so Container Apps saw no template change and kept revision 7 (model version 1) serving while the run reported success. The fix tags images with commit plus model version and makes the smoke test assert the served version; the corrected deploy is the next entry. | 2026-09-04 |
